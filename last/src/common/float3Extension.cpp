...
__host__ __device__ float3 Normalize(const float3& v) {
  ...
  if (mag2 > 0) {
    ...
    return make_float3(v.x * invMag, v.y * invMag, v.z * invMag);
  }
}
...
__host__ __device__ float3 UnitVector(float3 v) { return v / Length(v); }

__host__ __device__ float Distance(float3 a, float3 b) { return Length(a - b); }
