#ifndef GPU_FLOAT3EXTENSION_H
#define GPU_FLOAT3EXTENSION_H
#include <math.h>
...
__host__ __device__ std::ostream& operator<<(std::ostream& os, const float3& a);

__host__ __device__ float3 Normalize(const float3& v1);
...
inline __host__ __device__ std::ostream& operator<<(std::ostream& os,
                                                    const float3& a) {
  os << "[x " << a.x << ", y " << a.y << ", z " << a.z << "]\n";
  //  << ", time=" << r.t << "]\n";
  return os;
}
__host__ __device__ __inline__ float3 Maximum(const float3& a,
                                              const float3& b) {
  return make_float3(fmaxf(a.x, b.x), fmaxf(a.y, b.y), fmaxf(a.z, b.z));
}

__host__ __device__ __inline__ float3 Minimum(const float3& a,
                                              const float3& b) {
  return make_float3(fminf(a.x, b.x), fminf(a.y, b.y), fminf(a.z, b.z));
}

__host__ __device__ __inline__ float Get(const float3& a, const int i) {
  return i == 0 ? a.x : (i == 1 ? a.y : a.z);
}

#endif  // GPU_FLOAT3EXTENSION_H