...
void rst::rasterizer::rasterize_triangle(const Triangle &t, const std::array<Eigen::Vector3f, 3> &view_pos)
{
    ...
    // 画出三角形所在包围盒
    auto v = t.toVector4();
    float min_x = std::floor(std::min(v[0][0], std::min(v[1][0], v[2][0]))); // floor向下取整
    float max_x = std::floor(std::max(v[0][0], std::max(v[1][0], v[2][0]))); // ceil向上取整
    float min_y = std::floor(std::min(v[0][1], std::min(v[1][1], v[2][1])));
    float max_y = std::floor(std::max(v[0][1], std::max(v[1][1], v[2][1])));
