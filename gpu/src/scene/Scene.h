...
class HostScene {
 public:
  Camera *camera;
  static HostScene *_instance;
  std::vector<Triangle *> triangles;
  std::vector<Bounds3> aabbs;
  BVH *bvh;
  float3 lookat;
  Material *materials;
  int material_count;
...
//   bool trace(const  &ray, const std::vector<Object *> &objects, float &tNear,
...
// void HostScene::buildBVH() {
//   printf(" - Generating BVH...\n\n");
//   this->bvh = new BVHAccel(objects, 1, BVHAccel::SplitMethod::NAIVE);
// }

// Intersection HostScene::intersect(const Ray &ray) const {
//   return this->bvh->Intersect(ray);
// }

// void HostScene::sampleLight(Intersection &pos, float &pdf) const {
...
// bool HostScene::trace(const Ray &ray, const std::vector<Object *> &objects,
...
// Vector3f HostScene::castRay(const Ray &ray, int depth) const {
...
//     float lightDistance = diff.x * diff.x + diff.y * diff.y + diff.z *
//     diff.z;
...
//       L_dir = lightInter.emit * f_r * dotProduct(lightDir, N) *
//               dotProduct(-lightDir, NN) / lightDistance / pdf_light;
//     }
...
// void HostScene::buildScene(std::string path) {
...
//   MeshTriangle *floor = new MeshTriangle(path + "cornellbox/floor.obj",
//   white); MeshTriangle *tallbox =
//       new MeshTriangle(path + "cornellbox/tallbox.obj", white);
//   MeshTriangle *shortbox =
//       new MeshTriangle(path + "cornellbox/shortbox.obj", white);
//   // MeshTriangle *spotlight = new MeshTriangle(path +
//   "cornellbox/light.obj",
//   // &spotlightMaterial);
//   MeshTriangle *left = new MeshTriangle(path + "cornellbox/left.obj", red);
//   MeshTriangle *right = new MeshTriangle(path + "cornellbox/right.obj",
//   green); MeshTriangle *light_ = new MeshTriangle(path +
//   "cornellbox/light.obj", light); this->Add(floor); this->Add(shortbox);
//   this->Add(tallbox);
//   this->Add(left);
//   // this->Add(spotlight);
//   this->Add(right);
